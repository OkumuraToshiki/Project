...
{
	SetPolygonPos(tPolygon.fPos.x, tPolygon.fPos.y);							// 位置
	SetPolygonSize(tPolygon.fSize.x, tPolygon.fSize.y);							// サイズ
	SetPolygonAngle(tPolygon.fAngle);											// 角度
	SetPolygonTexture(pTexture);												// テクスチャー
	SetPolygonUV(tPolygon.fTexPos.x, tPolygon.fTexPos.y);						// UV座標
	SetPolygonFrameSize(tPolygon.fTexSize.x, tPolygon.fTexSize.y);				// テクスチャー1サイズ
	SetPolygonColor(tPolygon.fColor.x, tPolygon.fColor.y, tPolygon.fColor.z);	// 色
	SetPolygonAlpha(tPolygon.fColor.w);											// 透過度
