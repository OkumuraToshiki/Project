...
ID3D11DepthStencilState*	g_pDSS[2];				// Zバッファ/ステンシル ステート
D3D11_VIEWPORT vp;
...
void Stop_Max(int nVal, int nMax)
{
	if (nVal > nMax)
	{
		nVal = nMax;
...
void Stop_Min(int nVal, int nMin)
{
	if (nVal < nMin)
	{
		nVal = nMin;
...
//値ループ(プラス)
void LoopPlus(int &nVal, int nMaxCnt)
{
	nVal = (++nVal) % nMaxCnt;
}
//値ループ(マイナス)
void LoopMinus(int &nVal, int nMaxCnt)
{
