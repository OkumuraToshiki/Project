...
//2021/09/09->DrawMesh関数に自作クラス(Vector3f)追加
//=============================================================================
#include "mesh.h"
#include "Camera.h"
#include "Shader.h"
...
HRESULT InitMesh()
{
	...
	hr = LoadShader("Vertexmesh", "Pixelmesh",
	...
	bd.ByteWidth = sizeof(SHADER_MESH_GLOBAL);
	bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	bd.CPUAccessFlags = 0;
	hr = pDevice->CreateBuffer(&bd, nullptr, &g_pConstantBuffer[0]);
	if (FAILED(hr)) return hr;
	bd.ByteWidth = sizeof(SHADER_MESH_GLOBAL2);
...
// 描画処理
//=============================================================================
void DrawMesh(ID3D11DeviceContext* pDeviceContext, MESH* pMesh,LightClass*  pLight)
{
	// シェーダ設定
